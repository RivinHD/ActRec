...
def panel_factory(space_type: str):
    ...
    class AR_PT_local(Panel):
        ...
        def draw(self, context):
            ...
            box_row = box.row()
            col = box_row.column()
            col.template_list('AR_UL_locals', '', AR, 'local_actions',
                              AR, 'active_local_action_index', rows=4, sort_lock=True)
            col = box_row.column()
        ...
        def draw(self, context):
            ...
            if not AR.global_hide_menu:
                ...
                row2.operator("ar.global_rename",
                              text='Rename').label = AR.global_rename
        ...
        def draw(self, context):
            ...
            if AR.version != '':
                if AR.update:
                    ...
                    layout.label(text="latest Version (%s)" % AR.version)
    ...
    class AR_PT_advanced(Panel):
        ...
        def draw(self, context):
            ...
            row.prop(AR, 'show_all_categories', text="",
                     icon='RESTRICT_VIEW_OFF' if AR.show_all_categories else 'RESTRICT_VIEW_ON')
            ...
            row.prop(AR, 'autosave', toggle=True,
                     text="On" if AR.autosave else "Off")
            ...
            row.prop(AR, 'hide_local_text', text="", toggle=True,
                     icon="HIDE_ON" if AR.hide_local_text else "HIDE_OFF")
            col.prop(AR, 'local_create_empty',
