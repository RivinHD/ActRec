...
class AR_OT_Record_Execute(Operator):
    ...
    def execute(self, content):
        AR = bpy.context.preferences.addons[__package__].preferences
        Play(AR.Record_Coll[CheckCommand(self.index)].Command, self.index - 1)
        return {"FINISHED"}
...
class AR_OT_Command_Add(Operator):
    ...
    def execute(self, context):
        ...
        scene = context.scene
        if self.commands == "":
            message = Add(AR.Record_Coll[CheckCommand(0)].Index + 1)
        else:
            message = Add(AR.Record_Coll[CheckCommand(0)].Index + 1, self.commands)
        if message == "<Empty>":
            rec = AR.Record_Coll[CheckCommand(AR.Record_Coll[CheckCommand(0)].Index + 1)]
        ...
        if (AR.CreateEmpty and message) or not message:
            rec = AR.Record_Coll[CheckCommand(AR.Record_Coll[CheckCommand(0)].Index + 1)]
...
class AR_OT_Command_Remove(Operator):
    ...
    def execute(self, context):
        AR = bpy.context.preferences.addons[__package__].preferences
        scene = context.scene
        Remove(AR.Record_Coll[CheckCommand(0)].Index + 1)
        TempUpdateCommand(AR.Record_Coll[CheckCommand(0)].Index + 1)
        bpy.context.area.tag_redraw()
        SaveToDataHandler(None)
        return {"FINISHED"}
...
class AR_OT_Command_MoveUp(Operator):
    ...
    def execute(self, context):
        AR = bpy.context.preferences.addons[__package__].preferences
        scene = context.scene
        Move(AR.Record_Coll[CheckCommand(0)].Index + 1 , 'Up')
        TempUpdateCommand(AR.Record_Coll[CheckCommand(0)].Index + 1)
        bpy.context.area.tag_redraw()
        SaveToDataHandler(None)
        return {"FINISHED"}
...
class AR_OT_Command_MoveDown(Operator):
    ...
    def execute(self, context):
        AR = bpy.context.preferences.addons[__package__].preferences
        scene = context.scene
        Move(AR.Record_Coll[CheckCommand(0)].Index + 1 , 'Down')
        TempUpdateCommand(AR.Record_Coll[CheckCommand(0)].Index + 1)
        bpy.context.area.tag_redraw()
        SaveToDataHandler(None)
        return {"FINISHED"}
...
class AR_OT_Command_Clear(Operator):
    ...
    def execute(self, context):
        AR = bpy.context.preferences.addons[__package__].preferences
        scene = context.scene
        Clear(AR.Record_Coll[CheckCommand(0)].Index + 1)
        TempUpdateCommand(AR.Record_Coll[CheckCommand(0)].Index + 1)
        bpy.context.area.tag_redraw()
        SaveToDataHandler(None)
        return {"FINISHED"}
...
class AR_OT_Command_Edit(Operator):
    ...
    def execute(self, context):
        self.Edit = False
        AR = bpy.context.preferences.addons[__package__].preferences
        index_btn = AR.Record_Coll[CheckCommand(0)].Index + 1
        index_macro = AR.Record_Coll[CheckCommand(index_btn)].Index
        macro = AR.Record_Coll[CheckCommand(index_btn)].Command[index_macro]
        if self.CopyData:
            macro.macro = AR.LastLine
            macro.cname = AR.LastLineCmd
        else:
    ...
    def draw(self, context):
        ...
        command = ""
        if self.CopyData:
            layout.prop(AR, 'LastLine', text= "Name")
            command = AR.LastLineCmd
        else:
    ...
    def invoke(self, context, event):
        AR = context.preferences.addons[__package__].preferences
        index_btn = AR.Record_Coll[CheckCommand(0)].Index + 1
        macro = AR.Record_Coll[CheckCommand(index_btn)].Command[self.index]
        ...
        if self.last == mlast and cmd_edit_time[0] + 0.7 > t or self.Edit:
            ...
            self.last = mlast
            cmd_edit_time[0] = t
            AR.Record_Coll[CheckCommand(index_btn)].Index = self.index
        return {"FINISHED"}
...
class AR_OT_CopyToActRec(Operator):
    ...
    def poll(cls, context):
        AR = context.preferences.addons[__package__].preferences
        return context.active_object is not None and len(AR.Record_Coll[CheckCommand(0)].Command)
...
class AR_MT_Action_Pie(Menu):
    ...
    def draw(self, context):
        ...
        actions = AR.Record_Coll[CheckCommand(0)].Command
